class User {
  ...
  filterByTags(tagList) {
    ...
    const filteredWithoutDuplicates = [
      ...new Map(
        this.filteredFavoriteRecipes.map((recipe) => [recipe.id, recipe])
      ).values(),
    ];
  ...
  filterByKeyWord(userInput) {
    const filterByName = this.favoriteRecipes.filter(
      (recipe) =>
        recipe.name.toLowerCase().indexOf(userInput.toLowerCase()) !== -1
    );
