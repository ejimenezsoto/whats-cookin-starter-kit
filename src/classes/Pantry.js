class Pantry {
  ...
  listIngredientsNameAndAmount() {
    ...
    const ingredientDataObjs = ingredientIds.map((ingredientId) => {
      return [
        this.ingredientsData.find(({ id }) => id === ingredientId[0]),
        ingredientId[1],
      ];
    });
  ...
  checkPantry(recipeIngredients) {
    ...
    const filterThroughPantry = this.listOfPantryIngredients.reduce(
      (acc, ingredient) => {
    ...
    recipeKeys.forEach((recipeIngredient) => {
      if (!pantryKeys.includes(recipeIngredient)) {
        this.listOfMissingIngredients.push({
          name: recipeIngredient,
          amount: filterRecipes[recipeIngredient],
        });
      }
      pantryKeys.forEach((pantryIngredient) => {
        if (
          recipeIngredient === pantryIngredient &&
          filterThroughPantry[pantryIngredient] <
            filterRecipes[recipeIngredient]
        ) {
          this.listOfMissingIngredients.push({
            name: recipeIngredient,
            amount:
  ...
  getNewIngredients(currentUser) {
    const userData = fetch("http://localhost:3001/api/v1/users").then(
