const domUpdates = {
  ...
  displaySingleRecipe(
    ...
    <div class="single-recipe-img" tabindex="0">
        <img src="${findRecipeId.image}" alt=""> 
    </div>
    <div class="favorite-meal-buttons">
        <button class='favorite-button' id="favorite-button">
            <img class="favorite-image" id="favorite-button" src=${heartImage}  alt="favorite">
        </button>
        <button class='add-meal-button' id="add-meal-button">
            <img class="add-image" src="./images/plus.png" id="add-meal-button" alt="add">
        </button>
        <div class="total-cost">
          <h1 class="price">Total Cost $${findRecipeId.costOfIngredients()}</h1>
        </div> 
        </div>
  ...
  mealsToCookSingleRecipe(
      ...
      const pantryList = currentUser.pantry.listOfPantryIngredients.reduce(
        (acc, ingredient) => {
          if (ingredient.amount !== 0) {
            acc += `
        <li class='hidden'>${ingredient.name} Amount: ${ingredient.amount}</li>
    `;
          }
    ...
    <div class="favorite-meal-buttons" tabindex="0">
        <button class='shopping-cart-button'>
            <img class="shopping-cart-img" id="shoppingCart"src="./images/shopping-cart.png" alt="shopping Cart">
        </button>
        <button class='cook-button'>
            <img class="cook-img" src="./images/cooking.png" id="cookImg" alt="Cooking Img">
        </button>
        <div class="total-cost">
            <h1 class="price">Total Cost $${findRecipeId.costOfIngredients()}</h1>
        </div> 
    ...
    ${this.popUp(
      `You Are Missing ${missingIngredientsList}`,
      "Click the shopping cart to add ingredients",
      "myModal"
    )}
    ${this.popUp(
      `You have added these ingredients to your pantry ${missingIngredientsList}`,
      "Click the pan to cook a meal!",
      "successModal"
    )}
    ${this.popUp(
      `You have successfully cooked ${currentRecipe.name}`,
      "",
      "successfulCookModal"
    )}
    `;
